...
::dhA7pRFwIByZRRnk
::Zh4grVQjdCyDJGm222gVHDRncAWWMGqpFrkd8dTy//6R71sSUK8waoja1L2UJfIv0nDNNaU02HkagJ1YQh5Ae3I=
::YB416Ek+ZG8=
::
::
...
for /f "usebackq tokens=1,* delims==" %%A in ("%~dp0.env") do (
    if /i "%%A"=="TMDB_API_KEY" if not "%%B"=="" set TMDB_OK=1
    if /i "%%A"=="DB_HOST" if not "%%B"=="" set DB_HOST_OK=1
    if /i "%%A"=="DB_USER" if not "%%B"=="" set DB_USER_OK=1
)

REM Verifica se todas as variáveis estão preenchidas
if %TMDB_OK%==0 (
    echo A variavel TMDB_API_KEY nao esta preenchida no .env
)
if %DB_HOST_OK%==0 (
